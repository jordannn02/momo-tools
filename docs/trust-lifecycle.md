...
The report includes the manifest, missing files, mismatched files, per-artifact aliases, read/type failures, and whether an installed copy was checked. Run this strict comparison from a source checkout: an installed binary's own root resolves to that binary's canonical source plugin and is intentionally rejected. In strict mode, an omitted installed root, a supplied root that resolves to the canonical source plugin or repository tree, a symlinked installed/plugin root, a nested directory symlink, any final-file symlink (including one targeting an independent equal-byte file), a hardlink alias, a non-regular artifact such as a FIFO or device, missing artifacts, mismatched artifacts, or a source/installed read error fails the command. Inspection anchors traversal to open directory descriptors, validates every component without following symlinks, opens the final file with `O_NOFOLLOW`, `O_NONBLOCK`, and `O_CLOEXEC` when available, requires a regular file, compares the pre-open and descriptor metadata, hashes through that descriptor, then re-checks the named file and every directory component. Device, inode, size, mtime, and ctime must remain stable. The source-tree rejection uses `installed_root_is_canonical_source`; an omitted root uses `installed_root_required`; root and directory symlink failures are additive machine-readable errors, while `source_aliases`, `source_read_errors`, and `installed_read_errors` preserve artifact details.
...
`repair-plan` requires `--dry-run` and has no apply mode. Every validated doctor finding maps to one deterministic action. Unknown, unsafe, aliased, unstable, missing-proof, probe, or inconsistent-report findings are blocked by default. Expired or failed evidence can only recommend rerunning a bounded verification; it cannot extend validity or promote a verification level. Installed mismatch or missing-path advice requires an allowlisted public relative path, an explicitly named safe independent installed root, a current canonical source hash recomputed by the planner, and descriptor-anchored proof that the exact target's existing or absent state matches the finding while its directory chain remains stable. Every action remains `auto_applicable=false`, requires separate explicit authorization, and the report always records `applied=false` and `write_operations_executed=0`.
