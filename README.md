...
The trust commands operate only on the public package and, when supplied, one explicit installed-copy path. Run strict `integrity`, strict `slo`, strict `doctor`, and `recovery-drill` from a source checkout; the installed binary is smoke-tested with `test`. Strict comparison requires an independent installed copy and rejects the canonical source plugin or repository tree with `installed_root_is_canonical_source`, including when an installed binary is asked to compare its own root. It rejects symlinked installed/plugin roots, nested directory symlinks, hardlink aliases, and symlinks to independent equal-byte files. Artifact inspection anchors traversal to open directory descriptors, uses `lstat`, `O_NOFOLLOW`, non-blocking open, regular-file checks, stable identity, and post-hash pathname plus identity/size/mtime/ctime checks. Source or installed read/type failures are returned without a traceback. `integrity` compares hashes; it does not monitor or protect any real installation. `recovery-drill` creates, corrupts, and restores only a temporary copy. `slo` aggregates release checks. `doctor` aggregates public diagnostics but deliberately skips recovery, capability execution, private discovery, and external calls; failed evidence status and malformed, empty, blank-assertion, or assertionless public fixtures fail closed. `repair-plan --dry-run` has no apply mode, recomputes canonical hash proof and descriptor-anchored target-state proof before advisory reinstall output, and never writes.
